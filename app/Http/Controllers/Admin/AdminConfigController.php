...
class AdminConfigController extends Controller
{
    ...
    public function hub()
    {
        ...
        if ($mainCompany && $mainCompany->selflow_company_id && $mainCompany->selflow_sync_status === 'active') {
            try {
                ...
                // `company-info` de Selflow exige `X-Company-Key` : 403 si la
                // clé désigne un autre dossier, 401 si elle est absente,
                // inconnue ou révoquée. Sa tolérance de transition est tombée
                // avec les nôtres.
    ...
    public function importMapping($id)
    {
        ...
        $fieldsDictionary = [
            ...
            'courant' => [
                ...
                // Ventilation analytique portée directement par la ligne générale (format Sage réel :
                // "N° plan analytique" + "N° section"). Plusieurs paires possibles si la même ligne doit
                // être ventilée sur plusieurs axes en parallèle (ex: Point de vente + Projet en même temps),
                // chaque axe étant ventilé indépendamment à 100% du montant de la ligne.
                'axe_analytique' => [
                    'label' => 'N° Plan Analytique (Axe 1)',
                    'required' => false,
                    'icon' => 'fa-sitemap',
                    'match' => ['plan_analytique', 'axe', 'n_plan_analytique', 'plananalytique'],
                    'info' => 'Optionnel : code de l\'axe, utile seulement si plusieurs axes existent en base.'
                ],
                'section_analytique' => [
                    'label' => 'N° Section (Axe 1)',
                    'required' => false,
                    'icon' => 'fa-chart-pie',
                    'match' => ['section', 'n_section', 'nsection', 'section_analytique', 'centre_cout', 'pdv', 'point_vente'],
                    'info' => 'Code de la section (doit exister dans Analytique > Sections). Ventile la ligne à 100% sur cette section.'
                ],
                'axe_analytique_2' => [
                    'label' => 'N° Plan Analytique (Axe 2)',
                    'required' => false,
                    'icon' => 'fa-sitemap',
                    'match' => ['plan_analytique_2', 'axe_2', 'axe2'],
                    'info' => 'Optionnel : uniquement si un 2ème axe doit être ventilé en parallèle sur la même ligne.'
                ],
                'section_analytique_2' => [
                    'label' => 'N° Section (Axe 2)',
                    'required' => false,
                    'icon' => 'fa-chart-pie',
                    'match' => ['section_2', 'n_section_2', 'section2'],
                    'info' => 'Code de section pour le 2ème axe. Crée une ventilation supplémentaire, indépendante de l\'Axe 1.'
                ],
                'axe_analytique_3' => [
                    'label' => 'N° Plan Analytique (Axe 3)',
                    'required' => false,
                    'icon' => 'fa-sitemap',
                    'match' => ['plan_analytique_3', 'axe_3', 'axe3'],
                    'info' => 'Optionnel : uniquement si un 3ème axe doit être ventilé en parallèle sur la même ligne.'
                ],
                'section_analytique_3' => [
                    'label' => 'N° Section (Axe 3)',
                    'required' => false,
                    'icon' => 'fa-chart-pie',
                    'match' => ['section_3', 'n_section_3', 'section3'],
                    'info' => 'Code de section pour le 3ème axe. Crée une ventilation supplémentaire, indépendante des Axes 1 et 2.'
                ]
            ]
        ];
    ...
    public function importStaging($id, $returnData = false)
    {
        ...
        // --- SECTIONS ANALYTIQUES (pour la ventilation portée directement sur la ligne générale) ---
        $sectionsByCode = [];
        \App\Models\SectionAnalytique::where('company_id', $targetCompanyId)
            ->get(['id', 'code'])
            ->each(function ($s) use (&$sectionsByCode) {
                $code = strtoupper(trim($s->code));
                if ($code !== '') $sectionsByCode[$code] = $s->id;
            });
        ...
        foreach($data as $index => $rowRaw) {
            ...
            $processedRow = [
                ...
                'n_saisie' => null,
                'axe_analytique' => null,
                'section_analytique' => null,
                'axe_analytique_2' => null,
                'section_analytique_2' => null,
                'axe_analytique_3' => null,
                'section_analytique_3' => null,
            ];
            ...
            if ($import->type == 'initial') {
                ...
                // Information (non bloquante) : la ligne porte-t-elle une ou plusieurs ventilations analytiques ?
                // (une paire par axe potentiellement ventilé en parallèle sur cette même ligne)
                $sectionsResoluesApercu = [];
                foreach (['', '_2', '_3'] as $suffixe) {
                    $codeChamp = 'section_analytique' . $suffixe;
                    $sectionCodeApercu = strtoupper(trim((string)($row[$codeChamp] ?? '')));
                    if ($sectionCodeApercu === '') continue;

                    if (!isset($sectionsByCode[$sectionCodeApercu])) {
                        $errors[] = "Section analytique inconnue (axe" . ($suffixe ?: ' 1') . ") : {$sectionCodeApercu} (créez-la d'abord dans Analytique > Sections)";
                    } else {
                        $sectionsResoluesApercu[] = $sectionCodeApercu;
                    }
                }
                if (!empty($sectionsResoluesApercu)) {
                    $row['section_resolue_info'] = "Ventilée sur : " . implode(', ', $sectionsResoluesApercu);
                }
