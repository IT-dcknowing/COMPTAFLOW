...
class EntryController extends Controller
{
    ...
    public function indexBrouillons(Request $request)
    {
        $request->merge(['statut' => 'draft']);
        return $this->index($request);
    }

    public function indexRejetes(Request $request)
    {
        $request->merge(['statut' => 'rejected']);
        return $this->index($request);
    }
    ...
    /**
     * Enregistre une écriture complète (plusieurs lignes).
     */
    public function storeMultiple(Request $request)
    {
        ...
        $request->validate([
            ...
            'lines.*.description_operation' => 'required|string',
        ...
        try {
            return DB::transaction(function() use ($request, $user, $companyId, $status, $nSaisie, $exercice, $pieceFilename) {
                $created = [];
                foreach ($request->lines as $line) {
                    $created[] = EcritureComptable::create([
                        'company_id' => $companyId,
                        'user_id' => $user->id,
                        'n_saisie' => $nSaisie,
                        'code_journal_id' => $request->code_journal_id,
                        'exercices_comptables_id' => $exercice->id,
                        'date' => $request->date,
                        'description_operation' => $line['description_operation'],
                        'reference_piece' => $request->reference_piece,
                ...
                if ($status === 'pending') {
                    Approval::create([
                        'approvable_type' => EcritureComptable::class,
                        'approvable_id' => $created[0]->id, // On lie à la première ligne
                        'type' => 'accounting_entry',
                        'status' => 'pending',
                        'requested_by' => $user->id,
                        'data' => ['n_saisie' => $nSaisie, 'count' => count($created)]
                    ]);
                }

                return response()->json(['message' => 'Écriture enregistrée', 'n_saisie' => $nSaisie, 'count' => count($created)], 201);
            });
        } catch (\Exception $e) {
            Log::error('Erreur storeMultiple: ' . $e->getMessage());
            return response()->json([
                'message' => 'Échec lors de l\'enregistrement : ' . $e->getMessage()
            ], 500);
        }
    }
